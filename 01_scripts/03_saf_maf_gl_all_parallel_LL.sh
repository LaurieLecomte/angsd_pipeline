...
# parallel -a ./02_info/chr_list.txt -j 10 srun -p medium -c 2 --mem=80G --time=7-00:00 -J 03_saf_maf_gl_all_parallel_{} -o  log/03_saf_maf_gl_all_parallel_{}_%j.log /bin/sh 01_scripts/03_saf_maf_gl_all_parallel_LL.sh {} &
...
-r $REGIONS -out 03_saf_maf_gl_all/all_maf"$MIN_MAF"pctind"$PERCENT_IND"_maxdepth"$MAX_DEPTH_FACTOR"_no_outliers_"$REGIONS" #### ICI
...
gunzip 03_saf_maf_gl_all/all_maf"$MIN_MAF"pctind"$PERCENT_IND"_maxdepth"$MAX_DEPTH_FACTOR"_no_outliers_"$REGIONS".mafs.gz ## added _$REGIONS

INFILE=03_saf_maf_gl_all/all_maf"$MIN_MAF"pctind"$PERCENT_IND"_maxdepth"$MAX_DEPTH_FACTOR"_no_outliers_"$REGIONS".mafs ## added _$REGIONS
OUTFILE_sites=02_info/sites_all_maf"$MIN_MAF"pctind"$PERCENT_IND"_maxdepth"$MAX_DEPTH_FACTOR"_no_outliers_"$REGIONS" ## added _$REGIONS
OUTFILE_regions=02_info/regions_all_maf"$MIN_MAF"pctind"$PERCENT_IND"_maxdepth"$MAX_DEPTH_FACTOR"_no_outliers_"$REGIONS" ## added _$REGIONS
